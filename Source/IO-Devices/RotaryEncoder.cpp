...
RotaryEncoder::RotaryEncoder(IO_Checker* DeviceUpdater, RE_Pins RE, FX* audioEffect, LFO_Control_t ctrlParam, const char * Name = nullptr):bus(RE.D0,RE.D1, RE.D2, RE.D3,RE.D4,RE.D5, RE.D6, RE.D7){
    fx = audioEffect;   //set which effect the rotary encoder will control
    this->controlParam = ctrlParam;
    this->pos = 0;
    this->name = Name;
    DeviceUpdater->add_to_IO_check_list(this);
}

int RotaryEncoder::update_IO(){
...
void RotaryEncoder::controlFX(float scalar){
...
const char * RotaryEncoder::getName(){
    return this->name;
}
uint8_t RotaryEncoder::getPos(){
    return this->pos;
}
float RotaryEncoder::getPosf(){
    return ((float)this->pos)/127;
}
uint8_t RotaryEncoder::getGC(){
    return this->GC_latest;
}

uint8_t RotaryEncoder::convert_GreyCode(uint8_t GC){
        uint8_t p = 0;
    switch(GC){
        case 127:
            p = 0;
